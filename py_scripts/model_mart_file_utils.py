...
#     print("In model_mart_file_utils.py: Skipping model_mart_logging import: %s" % str(e))
...
def load_most_recent_df(df_load_fn, df_filepath, start_date, date_format_fn, max_retries=30):
    ...
    while retries <= max_retries:
        try:
            df = df_load_fn(str(df_filepath.fill(ParameterizedFileLoc.DATE, date_format_fn(day))))
            df.take(1)  # to force it to load the data now and fail early if the file doesn't exist

            logging.debug("Found data for day %s" % day)
            return df, day
        except Py4JError as e:
            logging.warning("Could not read data for day %s, skipping." % day)
            logging.debug(e)
